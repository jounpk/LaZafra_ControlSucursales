...
class Seguridad
{
    ...
    public function generaMenuLateral()
    {
      ...
      while ($dat=mysqli_fetch_array($res)){
		...
		#-----------------------  SECCIONES  ------------------------------
			public function validaSeccion($nombre)
			{
				#----------------------- SE VERIFICA QUE TENGA INFORMACION LA VARIABLE $nombre  ------------------------------
				if ($nombre != '') {
					require('../include/connect.php');
		      ...
		      $idSubMenu =$this->idSubMenu;
					...
					$filtroSubMenu = ($idSubMenu > 0 ) ? "AND ds.idSubMenu = '$idSubMenu'" : '' ;
					#----------------------- SE CONSULTA EN LA BD  ------------------------------
					$sqlSeccion="SELECT ds.link
												FROM segsecciones ds
												INNER JOIN segniveles sn ON ds.idNivel = sn.id
												WHERE ds.idNivel = '$level' AND ds.idArea = '$area' AND ds.nombre = '$nombre' AND sn.estatus = '1'
												ORDER BY ds.orden ASC
												LIMIT 1";
								//echo 'Menu de Usuario: '.$sqlSeccion.'<br>';
					//----------------devBug------------------------------
					if ($GLOBALS['devBug'] == 1) {
						$resSeccion= mysqli_query($link,$sqlSeccion) or die ("Error de consultar Menu Lateral: ".mysqli_error($link).'<br>SQL: '.$sqlSeccion);
						echo 'Sección de Usuario: '.$sqlSeccion.'<br>';
					} else {
						$resSeccion= mysqli_query($link,$sqlSeccion) or die (problemas('Se ha Bloqueado por Seguridad, por favor inténtalo de nuevo o notifica a tu Administrador... <br>Error: <b>SEG011</b>'));
					}
					//-------------Finaliza devBug------------------------------
					$cant = mysqli_num_rows($resSeccion);
					#----------------------- SE VERIFICA QUE HAYA RESULTADOS  ------------------------------
					if ($cant > 0) {
						$dat = mysqli_fetch_array($resSeccion);
						$d = $dat['link'];
						include "secciones/$d";
					}
