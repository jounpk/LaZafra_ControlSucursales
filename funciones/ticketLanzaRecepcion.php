...
########################## termina encabezado ##########################
    ...
    INNER JOIN (SELECT dr.idProducto, SUM(dr.cantidad) AS totales FROM detrecepciones dr WHERE dr.idRecepcion='$idRecepcion' GROUP BY dr.idProducto ) dtdo ON dtdo.idProducto= dtr.idProducto
    WHERE r.id = '$idRecepcion' AND dtr.estatus = 2
    ...
    $count = $nPag = 0;
      ...
      if ($idPro != $idProAnt) {
        $caracteres = 0;
        $caracteres = strlen($idPro);
        $cantLineas = abs($caracteres)/20;
  			$pdf->SetFont('Arial','',8);
  			$pdf->SetFont('Arial','B',9);
        $current_y = $pdf->GetY();
        $current_x = $pdf->GetX();
        $pdf->MultiCell(45,4,utf8_decode($idPro),0,'L');
        $pdf->SetXY($current_x + 45, $current_y);
        $pdf->Cell(25,4,utf8_decode(number_format($row['totales'],2,'.',',')),0,1,'R');

          if ($caracteres > 35) {
            $pdf->Ln(round($cantLineas+7));
