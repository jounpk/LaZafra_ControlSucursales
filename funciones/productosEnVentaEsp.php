...
$idSucursal=$_SESSION['LZFidSuc'];
	...
	$sqlProd = "SELECT d.*, p.costo,p.descripcion, s.cantActual, s.id AS idStock
									FROM ventas v
									...
									WHERE  v.estatus = '1' AND v.idSucursal = '$idSucursal' AND v.idUserReg = '$idUser' AND v.ventaEspecial = '1'
									ORDER BY d.id ASC";
	...
	if ($cantRows > 0) {
		while ($pd = mysqli_fetch_array($resProd)) {
			...
			$sqlLt = "SELECT idLote1,cant1,lote1,caducidad1,idLote2,cant2,lote2,caducidad2,idLote3,cant3,lote3,caducidad3
								FROM stocks s
								LEFT JOIN (SELECT id AS idLote1, cant AS cant1, lote AS lote1, caducidad AS caducidad1,idStock AS stock1 FROM lotestocks WHERE idStock = $idStock AND estatus = '1' AND cant > 0 ORDER BY caducidad ASC LIMIT 1) a ON a.stock1 = s.id
								LEFT JOIN (SELECT id AS idLote2, cant AS cant2, lote AS lote2, caducidad AS caducidad2,idStock AS stock2 FROM lotestocks WHERE idStock = $idStock AND estatus = '1' AND cant > 0 ORDER BY caducidad ASC LIMIT 1 OFFSET 1) b ON b.stock2 = s.id
								LEFT JOIN (SELECT id AS idLote3, cant AS cant3, lote AS lote3, caducidad AS caducidad3,idStock AS stock3 FROM lotestocks WHERE idStock = $idStock AND estatus = '1' AND cant > 0 ORDER BY caducidad ASC LIMIT 1 OFFSET 2) c ON c.stock3 = s.id
								WHERE s.id = $idStock";
			...
			$referencia = $precioProd = $lista = $descripcion = $cantidad = $cantActual = $acciones = $lote1 = $lote2 = $lote3 = $cantidad1 = $cantidad2 = $cantidad3 = $caducidad1 = $caducidad2 = $caducidad3 = $tsubtotal = '';
					$subtotal = $precioPd = 0;

					$descripcion = $pd['descripcion'];
					$referencia .= '<select id="precios" class="form-control" onChange="cambiaPrecioProd('.$pd['id'].',this.value,'.$pd['costo'].');">';
				...
				while ($pr = mysqli_fetch_array($resPrecios)) {
					$desactiva = ($pd['cantidad'] >= $pr['cantLibera']) ? '"' : 'text-dager" title="Disponible con cantidades de '.$pr['cantLibera'].' en adelante" disabled' ;
						$precioPd = $pr['precioProd'];
					$p1 = ($precioPd == $pd['precioVenta']) ? 'selected' : '' ;
					if ($precioPd > 0) {
						$referencia .= '<option value="'.$precioPd.'" '.$p1.' class="text-right '.$desactiva.'>$ '.number_format($precioPd,2,'.',',').'</option>';
					}
			...
			$referencia .= '</optgroup>
				...
				$referencia .= '</optgroup>
					...
					$min = $pd['costo'] + 2;
					$lista = $cont;
					$precioProd = '<input type="number" min="'.$min.'" class="text-right" value="'.$pd['precioVenta'].'" id="cant'.$pd['idVenta'].'" onChange="cambiaPrecioProd('.$pd['id'].',this.value,'.$min.');">';
					$cantidad = '<input type="number" step="any" min="0" max="'.$pd['cantActual'].'" class="form-control text-right" value="'.$pd['cantidad'].'" id="cant'.$pd['idVenta'].'" onChange="cambiaCantProd('.$pd['id'].',this.value,'.$pd['cantActual'].','.$pd['cantidad'].');">';
