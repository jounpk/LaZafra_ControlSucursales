...
LEFT JOIN segusuarios u3 ON c.idEntregaCorte = u3.id
WHERE c.idSucursal = '$idSuc' AND c.idRecoleccion > 0 AND DATE_FORMAT(d.fechaReg, '%Y-%m-%d') = '$fecha' LIMIT 1";
  //echo $sql;
  $result=mysqli_query($link,$sql) or die('Problemas al Consultar los Cortes.');
  $var=mysqli_fetch_array($result);
  ...
  $sql="SELECT c.id AS noCorte,c.montoEfectivo
        ...
        WHERE DATE_FORMAT(d.fechaReg, '%Y-%m-%d') = '$fecha' AND c.idSucursal = '$idSuc'";
        #echo '<br>$sql: '.$sql;
