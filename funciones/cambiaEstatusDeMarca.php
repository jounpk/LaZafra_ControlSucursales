...
if ($id == 0) {
  errorBD('No se reconoció la Marca, vuelve a intentarlo, si persiste notifica a tu Administrador');
}

if ($estatus == 0) {
  errorBD('No se reconoció el estatus de la Marca, vuelve a intentarlo, si persiste notifica a tu Administrador');
}

$newEstatus = ($estatus == 1) ? 2 : 1 ;

$sql = "UPDATE catmarcas SET estatus = '$newEstatus' WHERE id = '$id'";
