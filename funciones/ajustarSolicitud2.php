...
if ($cant > 0) {
    ...
    FROM
       ...
       AND lote.idSucursal='$sucursal'";
                ...
                if (isset($_POST[$cantlotes]) and $_POST[$cantlotes] != '0') {
                    ...
                    dtajs.idProducto= '$producto' WHERE dtajs.id='$iddetajuste'";
                        ...
                        if ($stock_actual >= $cantidad) {
                            ...
                            errorCarga('Lo sentimos el stock de su producto es insuficiente.');
                        }
                    }
                }
            }
            ...
            WHERE lote.idProducto="' . $dat["idProducto"] . '" AND ((lote.caducidad IS NULL AND lote.idSucursal="'.$sucursal.'") OR (lote.caducidad=(
          ...
          //  echo $sql;  
            $resCon = mysqli_query($link, $sql) or die(errorCarga('Problemas al consultar ajustes, notifica a tu Administrador'));
                ...
                //1. BUSCAR EL LOTE QUE TENGA LA MENOR CADUCIDAD
               //checar la chingadera
            /*   $sql = 'SELECT lote.id, lote.cant
        FROM lotestocks lote 
        WHERE lote.idProducto="' . $dat["idProducto"] . '" AND (lote.caducidad IS NULL AND lote.idSucursal="'.$sucursal.'") OR ( lote.caducidad=(
        SELECT MIN( lote.caducidad ) AS caducidad
        FROM detajustes det 
        INNER JOIN lotestocks lote ON lote.idProducto=det.idProducto AND lote.idProducto="' . $dat["idProducto"] . '"
        WHERE lote.idSucursal = "' . $sucursal . '"  ))
        ';*/
        $sql = "SELECT lote.id, lote.cant
        FROM lotestocks lote 
        ...
        if ($tipo_Ajuste == "1") {
           ...
           // echo ("ya entro");
            ...
            if ($stock_actual >= $cantidad) {
                ...
                // errorCarga('Lo sentimos el stock de su producto es insuficiente.'.$cantidad.'|'.$stock_actual.'|'.$sqlstock);
                errorCarga('Lo sentimos el stock de su producto es insuficiente.');
            }
        }
    } //while
