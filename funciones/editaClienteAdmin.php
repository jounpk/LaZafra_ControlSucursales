...
$limiteCredito = (!empty($_POST['eLimite'])) ? str_replace(',','',$_POST['eLimite']) : 0 ;
...
$sqlCon = "SELECT * FROM clientes WHERE nombre = '$nombre' AND rfc = '$rfc' AND id != '$ident'";
...
#echo '<br> -------------------- Si hay no hay un registro se ingresa. ----------------------- <br>';
    $sql = "INSERT INTO datosfisc(rfc,razonSocial,usoCfdi,calle,noExt,noInt,colonia,codigoPostal,municipio,estado,correo,correo2,correo3,
      verificaDatos,nombre,apellidos,portalCarga,idUserReg,fechaReg,tel,idEmpresa,estatus,idCliente)
        ...
        '0','$nombre',null,'1','$userReg',NOW(),null,'$idEmpresa','1','$ident')";
  }
#  echo '<br>$sql'.$sql;
...
$_SESSION['LZFmsjSuccessAdminClientes'] = 'El Cliente '.$nombre.' se ha editado correctamente';
...
#echo '<br> errorDatosFisc';
    $_SESSION['LZFmsjSuccessAdminClientes'] = 'El Cliente '.$nombre.' se ha editado correctamente'.$error;
    header('location: ../Administrador/clientes.php');
