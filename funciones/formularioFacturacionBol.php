...
<div class="alert alert-warning" role="alert">
  ...
  <p>Falta definición del módulo.</p>
  <hr>
  <p class="mb-0">Por favor, consulta a tu administrador.</p>
</div>
...
$resXFactHechas = mysqli_query($link, $sql) or die('Problemas al consultar las facturas, notifica a tu Administrador.');
$dat = mysqli_fetch_array($resXFactHechas);
$conteoFact = $dat["conteoFact"];
if ($conteoFact < 1 || $volverFacturar=='1' AND $devbug=='0') {
?>
  <div class="row">
    ...
    FROM
	  datosfisc dtsf";
        $resXBusqRFC = mysqli_query($link, $sql) or die('Problemas al consultar las facturas, notifica a tu Administrador.');
        echo "<option value=''>Selecciona un RFC</option>";
        while ($dat = mysqli_fetch_array($resXBusqRFC)) {
  ...
  <div class="row">
    ...
    <div class="col-md-8 col-sm-8">
      ...
      FROM
      datosfisc dtsf
      LEFT JOIN clientes cl ON dtsf.idCliente=cl.id";
        $resXBusqDtos = mysqli_query($link, $sql) or die('Problemas al consultar las facturas, notifica a tu Administrador.');
        echo "<option value=''>Selecciona un nombre</option>";
        while ($dat = mysqli_fetch_array($resXBusqDtos)) {
          echo "<option value='" . $dat["rfc"] . "'>" . $dat["nombreComp"] . "</option>";
        } ?>
      </select>
    </div>
          ...
          <div class='col-md-12'>
            ...
            <div class='input-group'>
              ...
              <select class='select2 form-control custom-select' id='cfdis' style="width:80%;" name='cfdi' required readonly>
                ...
                $resCFDI = mysqli_query($link, $sql) or die('Problemas al consultar CFDIS disponibles, notifica a tu Administrador.');
                while ($dat = mysqli_fetch_array($resCFDI)) {
...
$(document).ready(function(){
 $('#formCFDI select option:not(:selected)').attr('disabled',true);
    ...
    $("#formCFDI").submit(function(e) {
      ...
      $.post("../funciones/crearCFDIBol.php",
        datos,
        function(respuesta) {
    ...
    function recuperarCliente(id_value){
      $.post("../funciones/recuperarDatosCliente.php",
      {id:id_value},
      function(respuesta){
        var resp = respuesta.split('|');
...
<?php
} else if ($conteoFact >= 1 AND $devbug=='0') {
  $sql = "SELECT DISTINCT
  dts.rfc,
  dts.razonSocial,
  dts.correo,
...
?>
  ...
  <div class="row">
    <div class="col-md-5 col-sm-5">
      <a type="button" class="btn bg-success text-white" target="_blank" href="../<?= $xmlActual ?>"><i class="far fa-file-code"></i> Ver XML del CFDI </a>

    </div>

    <div class="col-md-5 col-sm-5">
      <a type="button" class="btn bg-danger text-white" target="_blank" href="../<?= $pdfActual ?>"><i class="fas fa-file-pdf"></i> Ver PDF del CFDI </a>
...
if (mysqli_num_rows($resXContComp)>0) {
  ...
  <script>
      ...
      {idVenta:idVenta, volverFacturar:'1'
            },
    function(respuesta){
