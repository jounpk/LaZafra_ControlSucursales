...
$sqlProductos = 'SELECT pro.id, pro.codBarra, pro.descripcion, dpto.nombre AS dpto, m.nombre AS marca, CONCAT("'.$img.'") AS foto,IF(SUM(stk.cantActual) IS NULL, 0,SUM(stk.cantActual))  AS stock, IF(pro.estatus=1,CONCAT("<center class=\"text-success ley_edicion\" id=\'btn-", pro.id, "\' data-toggle=\"tooltip\" data-placement=\"top\" title=\"\" data-original-title=\"Activo\"><button class=\"btn-circle\"  type=\"button\" ", IF(SUM(stk.cantActual)=0 OR SUM(stk.cantActual) IS NULL,"",""), " style=\"background:#fff; box-shadow: 7px 10px 12px -4px rgba(0,0,0,0.62);\" onclick=\"estatusPro(", pro.id,",0);\"
class=\"btn ink-reaction btn-icon-toggle\"><i class=\"fas fa-check text-success\"></i></button></center>"),
CONCAT("<center class=\"text-success ley_edicion\" id=\'btn-", pro.id, "\'  data-toggle=\"tooltip\" data-placement=\"top\" title=\"\" data-original-title=\"Desactivado\"><button class=\"btn-circle\"  type=\"button\" ", IF(SUM(stk.cantActual)=0 OR SUM(stk.cantActual) IS NULL,"",""), " style=\"background:#fff; box-shadow: 7px 10px 12px -4px rgba(0,0,0,0.62);\" onclick=\"estatusPro(", pro.id,",1);\"
