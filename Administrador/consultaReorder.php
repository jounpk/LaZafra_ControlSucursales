            ...
            <div class="card border-<?= $pyme; ?>">
              ...
              <div class="card-body">
                ...
                <div id="validation" class="m-t-40 jsgrid" style="position: relative; height: auto; width: 100%;">
                  ...
                  if (isset($_POST['busqMarcas']) and $_POST['busqMarcas'] >= 1) {
                    $busqMarcas = $_POST['busqMarcas'];

                    $filtroMrka = "AND pd.idCatMarca =" . $_POST['busqMarcas'];
                  } else {
                    $filtroDepto = '';
                    $busqMarcas = '';
                  }
                  ...
                  $listaMarcas='';
                  $listaMarcas='<option value="">Selecciona Una Marca</option>';

                  $sql="SELECT * FROM catmarcas WHERE estatus='1' ORDER BY nombre";
                  $result = mysqli_query($link, $sql) or die("Problemas al enlistar Marcas.");
                  while ($datos = mysqli_fetch_array($result)) {
                    $selectMrka=$datos['id']==$busqMarcas?'selected':'';
                    $listaMarcas.='<option value="'.$datos["id"].'" '.$selectMrka.'>'.$datos["nombre"].'</option>';
                  ...
                  $sqlLabel = "SELECT COUNT(stk.id) AS cantidad
                                          ...
                                          WHERE stk.idSucursal = '" . $_SESSION['LZFidSuc'] . "' AND stk.cantActual < stk.cantMinima
                                          	AND ($sqlMaxMin)
                                          	$filtroDepto 
                  ...
                  <div class="row">
                    <form method="post" action="consultaReorder.php">
                      ...
                      <div class="col-md-2">
                        ...
                        <div class="form-group">
                          ...
                          <div class="input-group">
                            ...
                            <select class="select2 form-control custom-select" name="busqMarcas" id="busqMarcas" onchange="" style="width: 80%;">
                              <?= $listaMarcas ?>
                            </select>
                          </div>
                        </div>
                      ...
                      <div class="col-md-3">
                        <div class="form-group">
                          ...
                          <div class="input-group">
                            ...
                            <select class="select2 form-control custom-select" name="buscaDepto" id="buscaDepto" onchange="" style="width: 70%;">
                  ...
                  <table class="table product-overview table-striped" id="tablaimpresion">
                    ...
                    <tbody>
                      ...
                      $sqlProductos = "SELECT stk.id, pd.descripcion AS producto, dpto.nombre AS depto, stk.cantActual, stk.cantMinima, stk.cantMaxima, bdg.cantActual AS cantBodega, stk.anaquel
                        ...
                        INNER JOIN productos pd ON stk.idProducto = pd.id
                        INNER JOIN departamentos dpto ON pd.idDepartamento = dpto.id
                        LEFT JOIN stocks bdg ON stk.idProducto = bdg.idProducto AND  bdg.idSucursal = '$idSucursal'
                        WHERE stk.idSucursal = '$idSucursal'
                        	AND ($sqlMaxMin)
                        	$filtroDepto $filtroMrka
                        ORDER BY dpto.nombre, pd.descripcion ASC";
                      // echo $sqlProductos;
