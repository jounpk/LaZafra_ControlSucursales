...
<body>
                                ...
                                ?>
                                        ...
                                        <form id="formTraspasos" method="post">
                                            <table class="table product-overview ">
                                                ...
                                                <tbody>
                                                    ...
                                                    prod.id AS idprod,
                                                    dtlt.id AS detlotes,
                                                    IF(dtlt.cantLotes IS NULL, '0',dtlt.cantLotes) AS cantidadLotes
                                                ...
                                                FROM
                                                    ...
                                                    INNER JOIN stocks sto ON det.idProducto = sto.idProducto 
                                                    AND sto.idSucursal = '$sucursal' 
                                                    LEFT JOIN (SELECT idDetTraspaso AS id, COUNT(id) AS cantLotes FROM dettrasplote GROUP BY(idDetTraspaso)) dtlt ON det.id=dtlt.id
                                                WHERE
                                                    det.idTraspaso = '$idTraspasos' 
                                                    AND tras.estatus = 1 
                                                    AND tras.idUserEnvio = '$userReg' 
                                                ORDER BY
                                                    id ASC;";
                                                    //echo $sql;
                                                    $it = 1;
                                                    ...
                                                    while ($datos = mysqli_fetch_array($res)) {
                                                        $color='';
                                                        if($datos['cantActual']<$datos['cantEnvio'] ){
                                                            $color='table-danger';

                                                        }
                                                        $checked = '';
                                                        if ($datos['cantidadLotes'] > 0) {
                                                            $encender .= 'cargalotes(' . $datos['idprod'] . ',' . $datos['cantEnvio'] . ',' . $datos['id'] . ');
                                                            ';
                                                            $checked = " checked ";
                                                        }
                                                        ...
                                                        echo '<tr id="tr' . $datos['idprod'] . '" class="'.$color.'">
                                                <td colspan="2"><input type="checkbox" data-producto="' . $datos['idprod'] . '" data-total="' . $datos['cantEnvio'] . '" data-iddettrasp="' . $datos['id'] . '" class="loteoCheck" ' . $checked . ' id="loteo_manual-' . $datos['idprod'] . '" name="chck_auto_' . $datos['id'] . '"  onclick="cargalotes(' . $datos['idprod'] . ',' . $datos['cantEnvio'] . ',' . $datos['id'] . ')" value="1"> </td>
                                    ...
                                    <div id="desbloquear-btn1">
                                        <div class="text-right">
                                            ...
                                            <button type="button" onclick="finalizaTraspaso('<?=$idTraspasos;?>')" class="btn btn-success ">Finalizar Traspaso</button>
  ...
  WHERE tras.idSucSalida ='$sucursal' AND tras.estatus = '1' AND sol.estatus='2'
   AND tras.idUserEnvio='0' ORDER BY fechaEnvio ASC";
        ...
        function cargalotes(ident, cantidad, idDetTrasp) {
            var debug = 0;
            if (debug == 1) {
                console.log("IDENT: " + ident);
                console.log("CANTIDAD: " + cantidad);
                console.log("ID DET TRASP: " + idDetTrasp);
            ...
            if ($('#loteo_manual-' + ident).is(':checked')) {
                ...
                $.post("../funciones/eliminaDetLotesTrasp.php", {
        ...
        function revisarCant(idProducto, idLote, valor) {
            ...
            var idDetTraspaso = $("#lote_" + idLote).data("dettrasp");
            var idLote = idLote;
            valorAgregado =$('#lote_'+idLote).val();
            if (debug == 1) {
                console.log("CANTIDAD MAXIMA A LLEGAR AL LOTE: " + cantMaxXLote);
                console.log("CANTIDAD TOTAL: " + cantTotalEnvio);
                console.log("Este Es valor Agreg. "+valorAgregado);
            ...
            if (parseInt(cantMaxXLote) < parseInt(valorAgregado)) {
                $("#lote_" + idLote).val(0);
                notificaBad("Verifica las Asignaciones de tus Lotes");
                $("#lote_" + idLote).focus();
                $("#area_resultado_" + idLote).html('');
                dispararLotes(idLote, 0, idDetTraspaso, 0);
                ...
                if (cantTotalEnvio < sumaTotales) {
                    ...
                    if (debug == 1) {
                        ...
                        console.log("ID DET TRASP : " + idDetTraspaso);
                    }

                    dispararLotes(idLote, valorAgregado, idDetTraspaso, 1);
                }

            }
        }

        function dispararLotes(idLote, valorAgregado, idDetTraspaso, operacion) {
            $.post("../funciones/registraDetLotesTrasp.php", {
                    idLote: idLote,
                    cantidad: valorAgregado,
                    idDetTrasp: idDetTraspaso
                },
                function(respuesta) {
                    var resp = respuesta.split('|');
                    if (resp[0] == 1) {
        ...
        function finalizaTraspaso(idTraspaso) {
            var debug = 1;
            clase = ".loteoCheck";
            errores=0;
            $(clase).each(function() {
                if ($(this).is(':checked')) {
                    ...
                    cantAEnviar = $(this).data("total");

                    $(".cant_loteo_" + producto).each(function() {
                        sumaTotales += parseFloat($(this).val());
                    ...
                    if (debug == 1) {
                        ...
                        console.log("SUMA DE LOTES ESCOGIDOS: " + sumaTotales);
                        console.log("CANT ENVIADA: " + cantAEnviar);
                    }
                    if (parseInt(sumaTotales) < parseInt(cantAEnviar)) {
                        notificaBad("Verifica Las Cantidades de Los Lotes Seleccionados");
            ...
            $.post("../funciones/loteoTrasp.php", {
                ...
                function(respuesta) {
                    ...
                    if (resp[0] == 1) {
                        notificaSuc(resp[1]);
                        $('<form action="../funciones/ticketLanzaTraspaso.php" method="POST"><input type="hidden" name="idTraspaso" value="' + idTraspaso + '"></form>').appendTo('body').submit();
